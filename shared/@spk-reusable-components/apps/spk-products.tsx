...
import ProductCardImageSwiper from "@/app/projects/components/store/cards/product/swiper";
import ProductPrice from "@/app/projects/components/store/product-page/product-info/product-price";
...
interface SpkProductsProps {
  ...
  weight?: number;
  images: VariantImageType[]; // VariantImages doğru şekilde burada tanımlanmalı
  variants: VariantSimplified[];
  setVariant: Dispatch<SetStateAction<VariantSimplified>>;
  selectedVariant: VariantSimplified;
}

const SpkProducts: React.FC<SpkProductsProps> = ({
  onclick,
  cartClick,
  detailsClick,
  card,
  ...
  const { name, slug, rating, sales, variantImages, id } = product;
    ...
    <Fragment>
      <div className="box card-style-2">
        <div className="box-body !p-0">
          <span
            className={`badge bg-${card.badgeColor}/[0.15] text-${card.badgeColor} top-left-badge`}
          >
            {card.badgetxt}
          </span>
          <div
            className={`badge top-right-badge bg-${card.rightbadgecolor} !text-white`}
          >
            <div className="badge-icon">
              <i className={`ti ti-${card.rightbadgeicon} text-[0.875rem]`}></i>
            </div>
                ...
                {variantImages && variantImages.length > 1 && (
                  <div className="flex flex-wrap gap-1">
                    {variantImages.map((image: any, index: number) => (
                      <Link
                        key={index}
                        href={image.url} // Burada image.url'yi kullanarak resme tıklanabilir bir bağlantı yapabilirsiniz
                        className={cn(
                        ...
                        onMouseEnter={() => setVariant(variants[index])} // Varyantları değiştir
                      >
                        <Image
                          src={image.image} // Görsel kaynağını alıyoruz
          ...
          <div className="p-3">
              ...
              <div className="min-w-fit">
                <div className="hs-tooltip ti-main-tooltip">
                  <Link
                    ...
                    className={`hs-tooltip-toggle btn btn-wishlist btn-icon rounded-circle ${card.active}`}
                  >
            ...
            <div className="flex align-items-baseline mt-4">
              ...
              <span className="text-[0.8125rem] ms-2 text-muted text-decoration-line-through">

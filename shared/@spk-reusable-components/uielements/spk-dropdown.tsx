        ...
        {Linktag ?
          <Link scroll={false} href="#!" className={Linkclass} aria-expanded={arialexpand} aria-label="anchor" id={buttonid}>
            {iconPosition === 'before' ?
              (
                <>
                  {Imagetag ? (<Image fill src={image} alt={Imagename} className={Imageclass} />) : <>{Icon ? (<i className={IconClass}></i>) : ""}</>}
                  {Toggletext}
                </>
              )
              : (
          ...
          <SpkButton size={Size} customClass={`ti-btn ${CustomToggleclass}`} variant={color} buttontype="button" Id={buttonid} Expand={arialexpand}>
            ...
            {Arrowicon ? <i className="ri-arrow-down-s-line align-middle ms-2 inline-block"></i> : ""}

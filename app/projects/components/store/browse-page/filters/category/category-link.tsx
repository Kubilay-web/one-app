    ...
    <div>
      <section>
        <div className="relative mt-2 flex w-full items-center justify-between leading-5">
          ...
          >
            <span className="relative mr-2 grid h-3 w-3 place-items-center rounded-full border border-[#ccc]">
              {category.url === categoryQuery && (
                <div className="inline-block h-1.5 w-1.5 rounded-full bg-black"></div>
              )}
            </span>
        ...
        {expand && (
          <>
            {category.subCategories?.map((sub) => (
              <section key={sub.id} className="relative mt-2 pl-5 leading-5">
                <label
                  htmlFor={sub.id}

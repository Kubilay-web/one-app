...
interface CheckoutStore {
  ...
  calculateShippingFee: (cartItems: any[], countryId: string, storeId?: string, currency?: string) => Promise<void>;
  setShippingFee: (fee: number) => void;
  setEstimatedDeliveryDays: (days: { min: number; max: number }) => void;
  setShippingService: (service: string) => void;
  resetShipping: () => void;

  placeOrder: (currency?: string) => Promise<{
    success: boolean;
    order?: any;
    paymentUrl?: string;
    sessionId?: string;
...
export const useCheckoutStore = create<CheckoutStore>((set, get) => ({
  ...
  fetchShippingAddresses: async () => {
    ...
    try {
      ...
      set({ error: error instanceof Error ? error.message : "Failed to fetch addresses", isLoading: false });
    }
  },
  ...
  addShippingAddress: async (address) => {
    ...
    try {
      ...
      set({ error: error instanceof Error ? error.message : "Failed to add address", isLoading: false });
    }
  },
  ...
  updateShippingAddress: async (id, address) => {
    ...
    try {
      ...
      const data = await response.json();
      set((state) => ({
        shippingAddresses: state.shippingAddresses.map((addr) => addr.id === id ? data.shippingAddress : addr),
        selectedAddressId: address.default ? id : state.selectedAddressId,
        isLoading: false,
      }));
    } catch (error) {
      set({ error: error instanceof Error ? error.message : "Failed to update address", isLoading: false });
    }
  },
  ...
  deleteShippingAddress: async (id: string) => {
    set({ isLoading: true, error: null });
    try {
      const response = await fetch(`/api/oneshop/checkout/shippingaddress?id=${id}`, { method: "DELETE" });
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || "Failed to delete address");
      }
      ...
      set((state) => {
        ...
        let newSelectedId = state.selectedAddressId;
        if (state.selectedAddressId === id) newSelectedId = addresses[0]?.id || null;
        return { shippingAddresses: addresses, selectedAddressId: newSelectedId, isLoading: false };
      });
      return data;
    } catch (error: any) {
  ...
  setSelectedAddress: (id) => {
    set({ selectedAddressId: id });
    const { cart } = useCartStore.getState();
    const { shippingAddresses, calculateShippingFee, currency } = get();
    const selectedAddress = shippingAddresses.find((addr) => addr.id === id);
    if (selectedAddress && selectedAddress.countryId && cart.length > 0) {
      calculateShippingFee(
        cart.map((item) => ({
  ...
  // Shipping fee calculation
  calculateShippingFee: async (cartItems, countryId, storeId, currency = "USD") => {
    set({ isLoading: true, error: null });
    try {
      const response = await fetch("/api/oneshop/checkout/calculateshipping", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ cartItems, countryId, storeId, currency }),
      });
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || "Failed to calculate shipping fee");
  ...
  // Order placement - CRITICAL FUNCTION
  placeOrder: async (currency = "USD") => {
    ...
    try {
      ...
      // Clear backend cart
      try {
        await fetch("/api/oneshop/cart", { method: "DELETE" });
      } catch (error) {
        console.error("Failed to clear backend cart:", error);
      }
      ...
      if (paymentMethod === "card" || paymentMethod === "paypal") {
        ...
        // COD/UPI için success response
        return {
          success: true,
          order: data.order,
          message: "Order placed successfully!",
          redirectUrl: `/shop/order/${data.order.id}`,
        };
      }
    } catch (error: any) {
      const errorMsg = error.message || "Failed to place order";
      set({ error: errorMsg, isProcessingPayment: false });
      return { success: false, error: errorMsg };
    } finally {

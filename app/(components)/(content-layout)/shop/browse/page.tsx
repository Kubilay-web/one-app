...
//     (key) =>
//       !["page", "sortBy", "minPrice", "maxPrice"].includes(key) &&
//       activeFilters[key]
//   ).length;
...
//     const baseUrl = `/shop/productdetails/${product.slug}`;
//     const variantSlug = product.variants[0]?.slug;
//     return variantSlug ? `${baseUrl}?variant=${variantSlug}` : baseUrl;
//   };
...
//                               key={item.brand}
//                               onClick={() =>
//                                 handleFilterChange("brand", item.brand)
//                               }
//                               className={`w-full text-start flex justify-between items-center p-2 rounded ${
...
//                               key={store.id}
//                               onClick={() =>
//                                 handleFilterChange("store", store.id)
//                               }
...
//                                     <i className="ti ti-store text-primary text-sm"></i>
//                                   </div>
//                                 )}
//                               </div>
...
//                                 } else if (key === "store") {
//                                   displayValue =
//                                     filters?.stores?.find((s) => s.id === value)
//                                       ?.name || value;
//                                 }
...
//                             className="form-control !py-2 pl-3 pr-10 appearance-none border rounded"
//                           >
...
//                           >
//                             <div className="box card-style-2 hover:shadow-lg transition-shadow duration-300 overflow-hidden h-full flex flex-col">
//                               <div className="box-body !p-0 flex flex-col flex-grow">
//                                 {isSale && (
...
//                                   </div>
//                                 )}
//                                 <div className="card-img-top border-b border-dashed border-defaultborder dark:border-defaultborder/10 relative flex-grow">
//                                   <Link
...
//                                     </div>
//                                   </div>
//                                   <div className="img-box-2 p-2 h-full">
//                                     <div className="relative w-full h-64 bg-light rounded overflow-hidden">
//                                       <Image
...
//                                           )}
//                                         </div>
//                                         <div className="inline-block text-primary text-[0.8125rem] font-semibold hover:underline truncate">
//                                           {product.store.name}
//                                         </div>
//                                       </div>
//                                       <h6 className="mb-1 font-semibold text-[1rem] line-clamp-2 min-h-[3rem]">
//                                         <Link
...
//                                     <div className="min-w-fit">
//                                       <div className="hs-tooltip ti-main-tooltip">
//                                         <button className="hs-tooltip-toggle btn btn-wishlist btn-icon rounded-circle">
...
//                                 <div className="mt-3 pt-3 border-t border-defaultborder/20">
//                                   <Link
//                                     href="/shop/cart"
//                                     className="ti-btn ti-btn-soft-primary ti-btn-lg !border-0 btn-style-1 !m-0 !rounded-tl-md rtl:!rounded-tr-md rtl:!rounded-tl-none w-full"
//                                   >
...
//   const [totalPages, setTotalPages] = useState(1);
//   const [searchQuery, setSearchQuery] = useState("");
//   const [tempSearchQuery, setTempSearchQuery] = useState(""); // Geçici search değeri
...
//       max: parseFloat(params.maxPrice || "10000"),
//     });
//     const urlSearchQuery = params.q || "";
//     setSearchQuery(urlSearchQuery);
//     setTempSearchQuery(urlSearchQuery);
//     setCurrentPage(parseInt(params.page || "1"));
//   }, [searchParams]);
...
//     // Sadece search butonuna tıklandığında veya Enter'a basıldığında arama yap
//     const newFilters = { ...activeFilters };
    
//     // Geçici search değerini asıl searchQuery'e aktar
//     setSearchQuery(tempSearchQuery);
    
//     if (tempSearchQuery) {
//       newFilters.q = tempSearchQuery;
//     } else {
//       delete newFilters.q;
//     }
...
//                               <label className="text-xs text-textmuted">Min</label>
//                               <input
//                                 type="number"
//                                 min="0"
//                                 max={filters.priceRange.max}
...
//                             </div>
//                             <div className="flex-1">
//                               <label className="text-xs text-textmuted">Max</label>
//                               <input
//                                 type="number"
//                                 min={priceRange.min}
//                                 max={filters.priceRange.max}
//                                 value={priceRange.max}
//                                 onChange={(e) => handlePriceChange(priceRange.min, parseInt(e.target.value) || filters.priceRange.max)}
//                                 className="form-control !py-1 text-sm"
//                               />
//                             </div>
//                           </div>
...
//                     {/* Categories Section with Clear Button */}
//                     {filters?.categories && filters.categories.length > 0 && (
//                       <div className="mb-6">
...
//                                     <span className="text-xs text-textmuted">Subcategories</span>
//                                     {activeFilters.subCategory && (
//                                       <button
...
//                     {/* Price Range Section with Clear Button */}
//                     {filters?.priceRange && (
//                       <div className="mb-6">
//                         <div className="flex justify-between items-center mb-3">
...
//                             <button
//                               onClick={() => handlePriceChange(0, filters.priceRange.max)}
//                               className="text-xs text-danger hover:text-danger-dark"
//                             >
//                               Clear
//                             </button>
...
//                     {/* Brands Section with Clear Button */}
//                     {filters?.brands && filters.brands.length > 0 && (
//                       <div className="mb-6">
//                         <div className="flex justify-between items-center mb-3">
...
//                     {/* Main Clear All Filters Button at Bottom */}
//                     {activeFilterCount > 0 && (
//                       <div className="mt-6 pt-4 border-t border-defaultborder/20">
//                         <button
//                           onClick={clearFilters}
...
import Image from "next/image";
import Link from "next/link";
import React, { Fragment, useEffect, useState, useCallback, useMemo, memo } from "react";
import { useSearchParams, useRouter } from "next/navigation";
import Seo from "@/shared/layouts-components/seo/seo";
import Pageheader from "@/shared/layouts-components/page-header/pageheader";
...
const ProductCard = memo(({ product, getProductLink, formatPrice }: { 
  product: Product; 
  ...
  const productImage = useMemo(() => 
    variant?.images[0]?.url || "/images/placeholder.png", 
    [variant]
  );
                  ...
                  <div className="min-w-fit">
                    {[...Array(5)].map((_, i) => (
                      <span key={i} className="text-warning me-1">
                        <i className={`bi bi-star${i < Math.floor(product.rating) ? "-fill" : i < product.rating ? "-half" : ""}`}></i>
                      </span>
                    ))}
                  </div>
                  <p className="mb-0 ms-1 min-w-fit text-textmuted dark:text-textmuted/50">
...
const BrowsePage = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [productsRes, filtersRes] = await Promise.all([
        fetch(`/api/oneshop/browse/products?${params}`, { signal: abortController.signal }),
        fetch("/api/oneshop/browse/filters", { signal: abortController.signal }),
      ]);
  ...
  const handleSearchSubmit = useCallback((e: React.FormEvent) => {
    ...
    setCurrentPage(1);
    updateURL(newFilters);
  }, [tempSearchQuery, activeFilters, updateURL]);
  ...
  const getProductLink = useCallback((product: Product) => {
    const baseUrl = `/shop/productdetails/${product.slug}`;
    const variantSlug = product.variants[0]?.slug;
    return variantSlug ? `${baseUrl}?variant=${variantSlug}` : baseUrl;
  }, []);
  ...
  const totalProductsCount = useMemo(() => 
    filters?.categories?.reduce((sum, cat) => sum + cat._count.products, 0) || 0,
    [filters]
  );
                        ...
                        {activeFilterCount > 0 && (
                          <div className="flex flex-wrap gap-2">
                            {Object.entries(activeFilters).map(([key, value]) => {
                              if (!value || ["page", "sortBy", "minPrice", "maxPrice"].includes(key))
                                return null;

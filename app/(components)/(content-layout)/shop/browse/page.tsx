...
const BrowsePage = () => {
  ...
  // URL parametrelerini state'e aktar
  useEffect(() => {
    const params = Object.fromEntries(searchParams.entries());
    setActiveFilters(params);
    setSortBy(params.sortBy || "newest");
  ...
  // Paralel fetch ile verileri çek
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
  ...
  const getProductPrice = useMemo(() => (product: Product) => {
    const variant = product.variants[0];
    if (!variant) return formatPrice(0);
  ...
  const getOriginalPrice = useMemo(() => (product: Product) => {
    const variant = product.variants[0];
    if (!variant) return null;
                          ...
                          >
                            <div className="box card-style-2 hover:shadow-lg transition-shadow duration-300 overflow-hidden h-full flex flex-col">
                              <div className="box-body !p-0 flex flex-col flex-grow">
                                ...
                                <div className="card-img-top border-b border-dashed border-defaultborder dark:border-defaultborder/10 relative">
                                  <Link
                                    href={getProductLink(product)}
                                    className="stretched-link"
                                  ></Link>

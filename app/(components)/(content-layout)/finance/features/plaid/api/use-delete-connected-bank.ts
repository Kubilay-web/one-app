...
type ResponseType = InferResponseType<typeof client.api.onefinance.plaid["connected-bank"]["$delete"], 200>;
...
export const useDeleteConnectedBank = () => {
  ...
  >({
    mutationFn: async () => {
      const response = await client.api.onefinance.plaid["connected-bank"].$delete();

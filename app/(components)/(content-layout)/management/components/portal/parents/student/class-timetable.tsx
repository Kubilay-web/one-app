...
// Define the structure of a single timetable row
...
// Define the structure of timetable data for a term
interface TermTimetable {
  [term: string]: TimetableRow[];
}

// Define the structure of all timetable data
interface TimetableRecord {
  [year: string]: TermTimetable;
}
...
export function ClassTimetable({ selectedTerm }: ClassTimetableProps) {
  // Type the mock data
  const timetableData: TimetableRecord = {
    "2023": {
      "1": [
        {
          time: "08:00 - 09:00",
  ...
  // Get the current term's timetable with proper type checking
  const currentTermTimetable: TimetableRow[] =
      ...
      <CardContent>
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {currentTermTimetable.map((row: TimetableRow, index: number) => (
                <TableRow key={index}>

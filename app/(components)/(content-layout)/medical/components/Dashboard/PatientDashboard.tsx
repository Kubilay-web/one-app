...
import { DoctorProps } from "../../(back)/dashboard/doctors/layout";
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Button asChild>
                <Link href="/medical/dashboard/user/appointments">View All</Link>
              </Button>
            </div>
          </CardHeader>
            ...
            {appointments &&
              appointments.slice(0, 5).map((item) => {
                return (
                  <Link
                    key={item.id}
                    href={`/medical/dashboard/user/appointments/view/${item.id}`}
                    className={
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Button asChild>
                <Link href="/medical/dashboard/doctor/patients">View All</Link>
              </Button>
            </div>
          </CardHeader>
            ...
            {doctors &&
              doctors.slice(0, 5).map((item) => {
                ...
                return (
                  <Link
                    key={item.doctorId}
                    href={`/medical/doctors/${slug}?id=${item.doctorId}`}
                    className={

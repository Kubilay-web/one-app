...
export default function InboxForm({
  ...
  async function onSubmit(data: InboxProps) {
    ...
    data.senderName = session?.user?.username ?? "";
    ...
    try {
      ...
      if (res.status === 201) {
        ...
        router.push(
          `/medical/dashboard/${
            session?.user.rolemedical === "DOCTOR" ? "doctor" : "user"
          }/medical/inbox`,
        );
      }
    } catch (error) {
    ...
    <div className="w-full max-w-2xl shadow-sm rounded-md m-3 border border-gray-200 mx-auto">
      <div className="text-center border-b border-gray-200 py-4 dark:border-slate-600">
        <div className="flex items-center justify-between px-6">
          ...
          <Button type="button" asChild variant={"outline"}>
            <Link href="/medical/dashboard/doctor/inbox">
              <X className="w-4 h-4" />
            </Link>
          </Button>
        </div>
      ...
      <form className=" py-4 px-4  mx-auto " onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="mt-8 flex justify-between gap-4 items-center">
          <Button type="button" asChild variant={"outline"}>
            <Link href="/medical/dashboard/doctor/inbox">Cancel</Link>
          </Button>

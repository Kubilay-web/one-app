...
//                         <Link
//                           href="/dashboards/ecommerce/admin/offer-tags/new"
...
//                                           <Link 
//                                             href={`/dashboards/ecommerce/admin/offer-tags/${tag.id}`}
...
//                                     </Link>
//                                     <Link
//                                       href={`/dashboards/ecommerce/admin/products?offerTag=${tag.id}`}
...
const AdminOfferTagsPage = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedTag, setSelectedTag] = useState<OfferTag | null>(null);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [bulkDeleteModalOpen, setBulkDeleteModalOpen] = useState(false);
  const [assignModalOpen, setAssignModalOpen] = useState(false);
  const [selectedTags, setSelectedTags] = useState<string[]>([]);
  ...
  // Open edit modal
  const openEditModal = (tag: OfferTag) => {
    setSelectedTag(tag);
    setEditFormData({
      id: tag.id,
      name: tag.name,
  ...
  const updateOfferTag = async () => {
    ...
    if (!editFormData.url.trim()) {
      toast.error('Tag URL is required');
      return;
    }

    // URL format validation
    const urlRegex = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
    if (!urlRegex.test(editFormData.url)) {
      toast.error('URL must contain only lowercase letters, numbers, and hyphens');
      return;
    }

    setEditLoading(true);
    try {
      const response = await fetch(`/api/oneshop/admin/offertags/main/${editFormData.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.success) {
        toast.success('Offer tag updated successfully');
        setEditModalOpen(false);
        fetchOfferTags();
      } else {
        toast.error(data.message || 'Failed to update offer tag');
      }
    } catch (error) {
      console.error('Error updating offer tag:', error);
      toast.error('Failed to update offer tag');
    } finally {
  ...
  return (
    <Fragment>
      <div className="main-content landing-main ecommerce-main">
                ...
                <div className="mb-6">
                  <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 sm:p-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div>
                        <h5 className="font-semibold text-xl mb-1">Manage Offer Tags</h5>
                      ...
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        ...
                        >
                          ...
                          <span>Add Offer Tag</span>
                        </Link>
                        {selectedTags.length > 0 && (
                          <>
                            <button
                              onClick={() => setBulkDeleteModalOpen(true)}
                              className="ti-btn ti-btn-danger flex items-center justify-center gap-2"
                            >
                              <i className="bi bi-trash text-sm"></i>
                              <span>Delete ({selectedTags.length})</span>
                            </button>
                            <button
                              onClick={() => setAssignModalOpen(true)}
                              className="ti-btn ti-btn-success flex items-center justify-center gap-2"
                            >
                              <i className="bi bi-link text-sm"></i>
                            ...
                            <th className="px-4 py-3 text-left text-sm font-semibold">
                              <div className="flex items-center gap-2">
                                <input
                                  type="checkbox"
                                  className="ti-form-checkbox"
                                  checked={selectedTags.length === offerTags.length && offerTags.length > 0}
                                  onChange={handleSelectAll}
                                />
                                <span className="hidden sm:inline">Offer Tag</span>
                              </div>
                            </th>
                            <th className="px-4 py-3 text-left text-sm font-semibold">
                              <button
                              ...
                              >
                                Products
                                {sortBy === 'productsCount' && (
                                  <i className={`bi bi-arrow-${sortOrder === 'asc' ? 'up' : 'down'} text-xs`}></i>
                                )}
                              </button>
                            </th>
                            <th className="px-4 py-3 text-left text-sm font-semibold hidden md:table-cell">Sample Products</th>
                            ...
                            >
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-3">
                                  <input
                                    type="checkbox"
                                    className="ti-form-checkbox flex-shrink-0"
                                    checked={selectedTags.includes(tag.id)}
                                    onChange={() => handleTagSelect(tag.id)}
                                  />
                                  <div className="flex items-center gap-3">
                              ...
                              <td className="px-4 py-3">
                                ...
                                {tag._count?.products > 0 && stats?.totalProductsWithTags > 0 && (
                                  <div className="text-xs text-gray-500 mt-1">
                                    {((tag._count.products / stats.totalProductsWithTags) * 100).toFixed(1)}% of tagged
                                  </div>
                                )}
                              </td>
                                ...
                                <div className="space-y-1">
                                  {tag.products?.slice(0, 3).map((product) => (
                                    <div key={product.id} className="flex items-center justify-between gap-2">
                                      <span className="text-sm truncate max-w-[180px]" title={product.name}>
                                        {product.name}
                                      </span>
                                      <div className="flex items-center gap-1 flex-shrink-0">
                                        ...
                                        <i className="bi bi-star-fill text-xs text-warning"></i>
                                      </div>
                                    </div>
                                  ))}
                                  {(!tag.products || tag.products.length === 0) && (
        ...
        {/* EDIT OFFER TAG MODAL - Responsive */}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg w-full max-w-2xl mx-auto my-8">
              <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h5 className="font-semibold text-xl">Edit Offer Tag</h5>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      Update offer tag information
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    This name will be displayed on product cards
                  </p>
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium mb-2">
                    Tag URL <span className="text-red-500">*</span>
                  </label>
                  <div className="flex items-center gap-2">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    URL will be used for filtering products (e.g., /offer/summer-sale)
                  </p>
                </div>
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    <i className="bi bi-info-circle text-blue-600 dark:text-blue-400 text-lg flex-shrink-0 mt-0.5"></i>
                    <div>
                      <p className="text-sm font-medium text-blue-800 dark:text-blue-300 mb-1">
                        About Offer Tags
                      </p>
                      <p className="text-xs text-blue-700 dark:text-blue-400">
                        Offer tags help highlight special promotions on products. 
                        When you assign this tag to products, it will appear as a badge 
                        on the product card and product detail page.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg w-full max-w-md mx-auto">
              <div className="p-6">
                ...
                <p className="mb-6 text-gray-600 dark:text-gray-300">
                  Are you sure you want to delete offer tag <strong className="font-semibold">"{selectedTag.name}"</strong>? 
                  It will be removed from all associated products.
                </p>
                <div className="flex flex-col sm:flex-row justify-end gap-3">
                  ...
                  <button
                    onClick={() => selectedTag && deleteOfferTag(selectedTag.id)}
        ...
        {/* Bulk Update Modal - Responsive */}
        {assignModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 overflow-y-auto">
            <div className="bg-white dark:bg-gray-800 rounded-lg w-full max-w-lg mx-auto my-8">
              <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
                    <i className="bi bi-pencil-square text-primary text-lg"></i>
                  </div>
                  <div>
                    <h5 className="font-semibold text-lg">Bulk Update</h5>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {selectedTags.length} offer tags selected
                    </p>
                  </div>
              ...
              <div className="p-6 space-y-6">
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium mb-2">New Name (Optional)</label>
                    <input
                      type="text"
                      className="form-control w-full"
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2">New URL Slug (Optional)</label>
                    <input
                      type="text"
                      className="form-control w-full"
                      placeholder="Leave empty to keep current URLs"
                      value={bulkUpdateData.url}
                      onChange={(e) => setBulkUpdateData(prev => ({ ...prev, url: e.target.value }))}
                    />
                    <p className="text-xs text-gray-500 mt-1">
                ...
                <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    <i className="bi bi-exclamation-triangle text-yellow-600 dark:text-yellow-400 text-lg flex-shrink-0 mt-0.5"></i>
                    <div>
                      <p className="text-sm font-medium text-yellow-800 dark:text-yellow-300 mb-1">
                        Note
                      </p>
                      <p className="text-xs text-yellow-700 dark:text-yellow-400">
                        Fields left empty will not be updated. Only the fields you fill will 
                        be applied to all selected offer tags.
                      </p>
                    </div>
                  </div>
                </div>

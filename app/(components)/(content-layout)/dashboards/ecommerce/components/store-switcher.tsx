...
import {
  ...
  PopoverContent,
  PopoverTrigger,
} from "@/app/projects/components/ui/popover";
...
const StoreSwitcher: FC<StoreSwitcherProps> = ({ stores, className }) => {
  ...
  const onStoreSelect = (store: { label: string; value: string }) => {
    setOpen(false);
    router.push(`/dashboards/ecommerce/seller/stores/${store.value}`);
  };
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-[250px] p-0 bg-gray-300">
        <Command>
          <CommandList>
          ...
          <CommandList>
            <CommandItem
              ...
              onSelect={() => {
                setOpen(false);
                router.push(`/dashboards/ecommerce/seller/stores/new`);
              }}
            >

...
const List = ({ token }) => {
  ...
  try {
    // 'id' parametresini URL'ye query string olarak ekliyoruz
    const response = await axios.delete(`${process.env.NEXT_PUBLIC_BASE_URL}/api/store/product?productId=${id}`, {
      headers: { token }
    });

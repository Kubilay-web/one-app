...
export function AppSidebar(props: React.ComponentProps<typeof Sidebar>) {
  ...
  const items: ItemType[] = [
    ...
    { title: "Availability", url: PROTECTED_ROUTES.AVAILBILITIY, icon: ClockIcon },
  ];
  ...
  return (
    <Sidebar
      collapsible="icon"
      variant="sidebar"
      className={`${state !== "collapsed" ? "w-[260px]" : ""} !bg-white !border-[#D4E162]`}
      {...props}
    >
      <SidebarHeader className={`!py-2 relative ${state !== "collapsed" ? "!px-5" : "!px-3"}`}>
          ...
          {items.map((item) => (
            <SidebarMenuItem key={item.title}>
              ...
              >
                <Link
                  href={item.url}
                  className="!text-[16px] !p-[12px_8px_12px_16px] min-h-[48px] rounded-[8px] !font-semibold flex items-center gap-2"
                >

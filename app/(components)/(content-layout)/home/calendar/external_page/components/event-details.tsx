...
const EventDetails = (props: EventDetailsProps) => {
  const { eventTitle, description, duration, username, user, eventLocationType } = props;

  const router = useRouter();
  const { timezone, hourType, next, isSuccess, selectedSlot, handleBack } = useBookingState();
  ...
  const locationOption = locationOptions.find(option => option.value === eventLocationType);
  ...
  return (
    ...
    >
      <div className="relative flex flex-row items-start justify-start md:justify-center md:flex-col z-10 p-6">
        <button
          type="button"
          onClick={handleClick}
          className="flex justify-center items-center cursor-pointer w-[43px] h-[43px]
                     border border-[rgba(26,26,26,0.1)] rounded-full 
                     bg-clip-padding text-[rgb(0,105,255)] text-[24px]"
        >
          <ArrowLeft />
        </button>

        <div className="flex flex-1 flex-col justify-start text-center md:justify-center md:text-left">
          <div className="text-muted-foreground capitalize mt-4 text-base font-semibold">
            {user?.name}
          </div>
            ...
            {next && (
              <div className="flex justify-start text-[15px] gap-2 items-start">
                ...
                <span className="font-medium">
                  {selectedSlot
                    ? formatSelectedSlot(selectedSlot, duration, timezone, hourType)
                    : "No slot selected"}
                </span>
              </div>
            )}

...
import type { InvoiceStatus, InvoiceTemplate, PaymentStatusInvoice } from "@prisma/client";
...
const UpdateUserAdminSchema = z.object({
  ...
  image: z.string().optional(),
  role: z
    .enum([UserRole.USER, UserRole.ADMIN, UserRole.SERVICE_PROVIDER])
    .optional(),
  status: z.string().optional(),
  isVerified: z.boolean().optional(),
});
...
export async function deleteUser(id: string) {
  try {
    ...
    // Check if user has permission (admin or deleting themselves)
    const hasPermission =
      currentUser.id === id || (await isAdmin(currentUser.id));
    ...
    // Just delete the user, all related records will be automatically deleted
...
export async function sendResetLink(email: string) {
  try {
    ...
    // Email sending code is commented out as in original
    // You can uncomment when ready to use Resend
...
export async function updateUserByAdmin(
  ...
  try {
    ...
    if (validatedData.firstName && validatedData.lastName) {
      ...
      // Get current user to combine
      const user = await db.user.findUnique({ where: { id: userId } });
      updateData.name = `${validatedData.firstName} ${
        user?.lastName || ""
      }`.trim();
    } else if (validatedData.lastName) {
      const user = await db.user.findUnique({ where: { id: userId } });
      updateData.name = `${user?.firstName || ""} ${
...
export async function getUserPlan(userId: string) {
  try {
    ...
    // Check if user has permission
    const hasPermission =
      currentUser.id === userId || (await isAdmin(currentUser.id));
...
export async function getUserStatistics() {
  try {
    ...
    const [totalUsers, activeUsers, usersByRole, usersByPlan, recentUsers] =
      await Promise.all([
        // Total users
        db.user.count(),
        ...
        // Users by subscription plan - using SubscriptionInvoice model
        db.subscriptionInvoice.groupBy({
          by: ["plan"],
          _count: {
            plan: true,
...
export async function upsertUserBrand(
  ...
  try {
    ...
    if (currentUser.id !== userId) {
      return {
        error: "Unauthorized to modify this brand",
      };
    }
...
export async function createInvoice(
  ...
  try {
    ...
    if (planInfo.remainingInvoices === 0) {
      return {
        error: "You have reached your daily invoice limit",
      };
    }
    ...
    // Update user's last invoice date
    await db.user.update({
      where: { id: userId },
      data: {
...
export async function updateInvoiceStatus(
  ...
  try {
    ...
    const invoice = await db.invoiceModel.findUnique({
      where: { id: invoiceId },
      select: { userId: true },
    });
...
export async function getInvoiceById(invoiceId: string) {
  try {
    ...
    // Check if user has permission
    const hasPermission =
      invoice.userId === currentUser.id || (await isAdmin(currentUser.id));

...
import FilterBar from "./filter-bar";
import TableActions from "./table-actions";
import { DateRange, DateFilterOption } from "../../../components/ui/data-table/date-filter";
...
export default function DataTable<T>({
  ...
  // Apply date filter
  const applyDateFilter = (items: T[]): T[] => {
    if (!dateFilter.range?.from || !dateFilter.range?.to || !filters?.getItemDate) return items;
    const from = new Date(dateFilter.range.from);
    const to = new Date(dateFilter.range.to);
    return items.filter((item) => {
  ...
  const filteredData = Array.isArray(applyDateFilter(applySearchFilter(data)))
    ? applyDateFilter(applySearchFilter(data))
    : [];
    ...
    <Card className="w-full">
      ...
      <CardContent>
            ...
            {currentItems.length > 0 ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length + (renderRowActions ? 1 : 0)} className="text-center py-6">
                  {emptyState || (searchQuery || dateFilter.option !== "lifetime" ? "No matching items found" : "No items found")}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
        ...
        {filteredData.length > 0 && (
          <div className="mt-4 flex flex-col sm:flex-row justify-between items-center">
            <div className="mb-2 sm:mb-0">
              <RowsPerPage value={itemsPerPage} onChange={setItemsPerPage} options={[5, 10, 25, 50, 100]} />
            </div>
            <div className="text-sm text-muted-foreground">
                ...
                <PaginationContent>
                  ...
                  {getPageNumbers().map((page, index) =>
                    page === "ellipsis" ? (
                      ...
                      <PaginationItem key={`page-${page}`}>
                        <PaginationLink
                          onClick={() => handlePageChange(page as number)}
                          className={clsx(currentPage === page ? "bg-primary text-primary-foreground" : "cursor-pointer")}
                        >
                          {page}
                        </PaginationLink>
                      </PaginationItem>
                  ...
                  <PaginationItem>
                    <PaginationNext
                      ...
                      className={clsx(currentPage === totalPages ? "pointer-events-none opacity-50" : "cursor-pointer")}
                    />
                  </PaginationItem>
                </PaginationContent>
              </Pagination>
